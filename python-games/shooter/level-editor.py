...
def draw_grid():
    ...
    for c in range(ROWS):
        pygame.draw.line(screen,WHITE,(0, c * TILE_SIZE),(SCREEN_WIDTH + 8, c * TILE_SIZE))
...
save_button = button.Button(SCREEN_WIDTH // 2 - 100, SCREEN_HEIGHT + LOWER_MARGIN - 100, save_img, 1)
load_button = button.Button(SCREEN_WIDTH // 2 + 100, SCREEN_HEIGHT + LOWER_MARGIN - 100, load_img, 1)
...
while run:
    ...
    draw_grid()
    draw_world()
    draw_text(f'Level: {level}', font, WHITE, 10, SCREEN_HEIGHT + LOWER_MARGIN - 150)
    draw_text('Press UP or DOWN to change level', font, WHITE, 10, SCREEN_HEIGHT + LOWER_MARGIN - 120)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_UP and level < 4:
                level += 1
